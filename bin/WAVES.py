...
if hasattr(sys, "frozen"):
    ...
    if os.path.exists(os.path.join(parent_dir, "WAVESCore")):
        sys.path.append(parent_dir)
    CONF_DIR = os.path.dirname(os.path.join(parent_dir,'WAVESCore/'))
    CONF_DIR = os.path.dirname(os.path.join(CONF_DIR, 'config'))


from WAVESCore.net import HTTP, lswww
from WAVESCore.file.reportgeneratorsxml import ReportGeneratorsXMLParser
from WAVESCore.file.vulnerabilityxmlparser import VulnerabilityXMLParser
from WAVESCore.file.anomalyxmlparser import AnomalyXMLParser
from WAVESCore.net.crawlerpersister import CrawlerPersister
...
class WAVES(object):
    ...
    Launch WAVES without arguments or with the "-h" option for more informations."""

    REPORT_DIR = "report"
    REPORT_FILE = "vulnerabilities"
    HOME_DIR = os.getenv('HOME') or os.getenv('USERPROFILE')
    COPY_REPORT_DIR = os.path.join(HOME_DIR, ".WAVES", "generated_report")
    ...
    def __initAttacks(self):
        ...
        from WAVESCore.attack import attack

        print(("[*] Loading modules:"))
        print(u"\t {0}".format(u", ".join(attack.modules)))
        for mod_name in attack.modules:
            mod = __import__("WAVESCore.attack." + mod_name, fromlist=attack.modules)
    ...
    def attack(self):
        ...
        if self.http_engine.sslErrorOccured:
            print('')
            print("Warning: WAVES came across some SSL errors during the scan, it maybe missed some webpages.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="WAVES: Web application vulnerability scanner", add_help=False)
        ...
        parser.add_argument('--version', action='version', version=WAVES_VERSION)
        ...
        if attackFile is not None:
            if crawlerPersister.isDataForUrl(attackFile) == 1:
                ...
                print ("File {0} loaded. WAVES will use it to perform the attack".format(attackFile))
            else:
                print("File {0} not found. WAVES will scan the web site again".format(attackFile))
        ...
        try:
            ...
            print(("Attack process interrupted. To perform again the attack, "
                    "lauch WAVES with \"-i\" or \"-k\" parameter."))
            print('')
            pass
    except SystemExit:
        pass